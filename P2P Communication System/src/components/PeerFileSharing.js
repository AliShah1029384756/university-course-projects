...
import { usePeerFiles } from '../context/PeerFileContext';

const PeerFileSharing = ({ peerConnections }) => {
    const { peerFiles, updatePeerFiles } = usePeerFiles();
    ...
    const requestFilesFromPeers = () => {
        peerConnections.forEach(conn => {
            if (conn.open) {
                conn.send({ type: 'request-peer-files' });
            }
        });
    };
...
export default PeerFileSharing;